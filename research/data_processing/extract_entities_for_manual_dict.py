    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if label.startswith('S-'):
                if current_entity and current_type:
                    entities.append((''.join(current_entity), current_type))
                entity_type = label[2:]  # 提取类型，如 S-PER -> PER
                entities.append((char, entity_type))
                current_entity = []
                current_type = None
...
def main():
    parser = argparse.ArgumentParser(description='从 BMES 标注提取实体生成手动专家词典（带类型）')
    ...
    parser.add_argument('--with_type', action='store_true', default=True,
                        help='输出带类型的词典（word\\ttype 格式）')
    
    args = parser.parse_args()
    
    print("="*70)
    print("从 BMES 标注提取实体生成手动专家词典（带类型）")
    print("="*70)
    ...
    with open(args.output_path, 'w', encoding='utf-8') as f:
        for (entity, etype), freq in sorted_entities:
            if args.with_type:
    ...
    # 类型分布
    print("\n类型分布:")
    type_dist = Counter(etype for (_, etype) in filtered_entities.keys())
    for etype, count in sorted(type_dist.items(), key=lambda x: x[1], reverse=True):
        print(f"  {etype:8s}: {count:,} 个")
    
    # 词长分布
    print("\n词长分布:")
    length_dist = Counter(len(entity) for (entity, _) in filtered_entities.keys())
    for length in sorted(length_dist.keys()):
        print(f"  {length}字词: {length_dist[length]:,} 个")
    
    # Top 20 高频实体
    print("\nTop 20 高频实体:")
    for i, ((entity, etype), freq) in enumerate(sorted_entities[:20], 1):
        print(f"  {i:2d}. {entity:10s} [{etype:4s}] (频次: {freq:,})")
