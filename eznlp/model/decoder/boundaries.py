...
class Boundaries(TargetWrapper):
    ...
    def __init__(
        ...
        self.max_span_size = min(
            getattr(config, "max_span_size", self.num_tokens), self.num_tokens
        )

        if (
            getattr(config, "inex_mkmmd_lambda", 0.0) > 0
            or config.nested_sampling_rate < 1
        ):
            self.nest_non_mask = torch.zeros(
                self.num_tokens, self.num_tokens, dtype=torch.bool
            )
        ...
        if training and (
            config.neg_sampling_rate < 1
            or config.neg_sampling_power_decay > 0
            or config.nested_sampling_rate < 1
        ):
            span_size_ids = torch.arange(self.num_tokens) - torch.arange(
                self.num_tokens
            ).unsqueeze(-1)
            non_mask_rate = config.neg_sampling_rate * (span_size_ids + 1) ** (
                -config.neg_sampling_power_decay
            )
            ...
            if config.neg_sampling_surr_rate > 0 and config.neg_sampling_surr_size > 0:
                ...
                for label, start, end in self.chunks:
                    for dist in range(1, config.neg_sampling_surr_size + 1):
                        for surr_start, surr_end in _spans_from_surrounding(
                            (start, end), dist, self.num_tokens
                        ):
                            surr_non_mask[surr_start, surr_end - 1] = True
                non_mask_rate[surr_non_mask] += (
                    1 - non_mask_rate[surr_non_mask]
                ) * config.neg_sampling_surr_rate
        ...
        if self.chunks is not None:
            if (
                (not config.multilabel)
                and config.sb_epsilon <= 0
                and config.sl_epsilon <= 0
            ):
                # Cross entropy loss for non-smoothing
                self.label_ids = torch.full(
                ...
                self.label_ids = torch.zeros(
                    self.num_tokens, self.num_tokens, config.voc_dim, dtype=torch.float
                )
                for label, start, end in self.chunks:
                    ...
                    for dist in range(1, _sb_size + 1):
                        ...
                        for sur_start, sur_end in sur_spans:
                            self.label_ids[sur_start, sur_end - 1, label_id] += (
                                eps_per_span * config.sb_adj_factor
                            )
                        # Absorb the probabilities assigned to illegal positions
                        self.label_ids[start, end - 1, label_id] += eps_per_span * (
                            dist * 4 - len(sur_spans)
                        )
                ...
                if not config.multilabel:
                    ...
                    if overflow_indic.any().item():
                        self.label_ids[overflow_indic] = torch.nn.functional.normalize(
                            self.label_ids[overflow_indic], p=1, dim=-1
                        )
                    self.label_ids[:, :, config.none_idx] = 1 - self.label_ids.sum(
                        dim=-1
                    )
                else:
                    self.label_ids[:, :, config.none_idx] = 1 - self.label_ids.sum(
                ...
                if config.sl_epsilon > 0:
                    ...
                    self.label_ids[:, :, pos_indic] = self.label_ids[
                        :, :, pos_indic
                    ] * (1 - config.sl_epsilon) + self.label_ids[:, :, pos_indic].sum(
                        dim=-1, keepdim=True
                    ) * config.sl_epsilon / (
                        config.voc_dim - 1
                    )
    ...
    def __getattr__(self, name):
        if name.startswith("diagonal_"):
            ...
            diagonal_x = torch.cat(
                [x.diagonal(offset=k - 1) for k in range(1, self.max_span_size + 1)],
                dim=-1,
            )
            if diagonal_x.dim() == 2:
...
class DiagBoundariesPairs(TargetWrapper):
    ...
    def chunks_pred(self, chunks: List[tuple]):
        ...
        if self.chunks_pred is not None:
            ...
            self.chunks = self.chunks_gold + [
                ck for ck in self.chunks_pred if ck not in self.chunks_gold
            ]
    ...
    def build(self, config: Union[SingleDecoderConfigBase, DecoderBase]):
        ...
        if self.relations is not None:
            self.dbp2label_id = torch.full(
                (num_spans, num_spans), config.none_idx, dtype=torch.long
            )
            for label, (_, h_start, h_end), (_, t_start, t_end) in self.relations:
                if (
                    h_end - h_start <= self.max_span_size
