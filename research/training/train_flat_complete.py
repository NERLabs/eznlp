...
使用方式:
    python train_flat_complete.py --data_dir datasets/raw/RedJujube --word_file assets/vectors/ctb.50d.vec
"""
import argparse
import os
import sys
...
# 导入 FLAT 组件
from research.model_variants.models.flat_data_processor import FLATDataProcessor, FLATDataset, load_word_list
from research.model_variants.models.flat_extractor import FLATModel, FLATModelWithBERT, FLATWithInterAttention
...
def build_optimizer_and_scheduler(model, num_train_batches: int, args, bert_model=None, model_type='flat'):
    """构建优化器和调度器（对齐 research.tools/utils.build_trainer 的策略）"""
