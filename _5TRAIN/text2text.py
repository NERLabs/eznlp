...
from eznlp.model import EncoderConfig, GeneratorConfig, OneHotConfig, Text2TextConfig
...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_data.add_argument(
        "--dataset", type=str, default="multi30k", help="dataset name"
    )
    ...
    group_decoder.add_argument(
        ...
        type=str,
        default="LSTM",
        choices=["LSTM", "GRU", "Gehring", "Transformer"],
        help="token-level decoder architecture",
    )
    group_decoder.add_argument(
        "--atten_num_heads", type=int, default=1, help="attention number of heads"
    )
    group_decoder.add_argument(
        "--atten_scoring", type=str, default="additive", help="attention scoring"
    )
    group_decoder.add_argument(
        "--sin_positional_emb",
        default=False,
    ...
    group_decoder.add_argument(
        "--teacher_forcing_rate", type=float, default=0.5, help="teacher forcing rate"
    )
    group_decoder.add_argument(
        "--init_ctx_mode",
        type=str,
...
def collect_T2T_assembly_config(args: argparse.Namespace):
    ...
    emb_config = OneHotConfig(
        ...
        sin_positional_emb=args.sin_positional_emb,
    )
    ...
    gen_config = GeneratorConfig(
        arch=args.dec_arch,
        embedding=trg_emb_config,
        hid_dim=args.hid_dim,
        num_layers=args.num_layers,
        in_drop_rates=drop_rates,
        fl_gamma=args.fl_gamma,
        sl_epsilon=args.sl_epsilon,
        num_heads=args.atten_num_heads,
        scoring=args.atten_scoring,
        weight_tying=args.use_weight_tying,
        teacher_forcing_rate=args.teacher_forcing_rate,
        init_ctx_mode=args.init_ctx_mode,
        ff_dim=args.ff_dim,
    )

    return {"embedder": emb_config, "encoder": enc_config, "decoder": gen_config}
