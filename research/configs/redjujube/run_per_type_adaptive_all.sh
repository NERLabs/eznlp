...
LOG_FILE="logs/research_logs/per_type_adaptive_all.log"
...
for seed in "${SEEDS_PUBLIC[@]}"; do
  run_general "boson" "datasets/raw/boson/boson.train.bmes" "datasets/raw/boson/boson.dev.bmes" "datasets/raw/boson/boson.test.bmes" "$LEX_BOSON" "$seed"
done

for seed in "${SEEDS_PUBLIC[@]}"; do
  run_general "clue" "datasets/raw/clue/train.char.bmes" "datasets/raw/clue/dev.char.bmes" "datasets/raw/clue/test.char.bmes" "$LEX_CLUE" "$seed"
done
