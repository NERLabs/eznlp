...
状态：已完成图片质量审查；当前记录作为后续重画图 1-5 的执行规格和验收依据。
...
| 尺寸 | 通过，PNG 尺寸为 2200-2400 px 宽，满足打印清晰度 |
...
| PNG 尺寸 | 图 1/5 为 2400 x 1240；图 2 为 2200 x 1120；图 3 为 2200 x 1200；图 4 为 2400 x 1360 |
...
|---|---|---|---|---|---:|
| 图 1 | EDBP 总体结构 | 清晰度合格 | 只有矩形框和箭头，层级关系偏平，缺少输入、编码、解码、损失之间的分区背景 | 是 | 1 |
| 图 2 | BMES 词典编码 | 清晰度合格 | 流程过抽象，没有展示真实红枣术语样例，难以体现 BMES 四通道如何落到字符位置 | 是 | 2 |
| 图 3 | 边界预测解码器 | 清晰度合格 | span 矩阵过小，biaffine 公式和候选边界含义没有视觉强化 | 建议 | 3 |
| 图 4 | 各类别 F1 | 清晰度合格 | 柱状图可读但较朴素，类别缩写未直接解释，低频类别差异没有突出样本量或类别组别 | 建议 | 4 |
| 图 5 | 错误案例 | 清晰度合格 | 更像文本卡片，缺少金标与预测边界的对照线或 token 级标注，论文图感较弱 | 是 | 2 |
...
现有图片不会导致清晰度或嵌图校验失败，但会削弱论文的专业观感。若按最终投稿标准，建议至少重画图 1、图 2 和图 5，再重新生成 PNG、DOCX 和 PDF。

本轮审图后的结论是：当前图片可以作为论文结构和内容的技术占位，但不应作为最终投稿图件。最终投稿前必须至少完成图 1、图 2 和图 5 的重画；图 3 和图 4 可以在版式时间不足时保留，但仍建议做一致风格修订。
