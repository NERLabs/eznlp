...
与 `Boundary Smoothing` 四组当前路径 RJND/RedJujube seed=42 实验。
`results.json` 只保存 loss/F1，Precision 和
...
- `Boundary Smoothing` 旧候选位于
  ...
  当前路径证据。
- `SoftLexicon` seed=42 可定位到
  `experiments/EXP-010-optimization/results_newdata/SoftLexicon_baseline/seed_42/softlexicon_20260421-212809/results.json`，
  但其数据路径为 `_2DATA/RedJujube`，词典路径为 `data/HZ/softlexicon_train.txt`；
  该结果只能带限制说明使用，严格 RJND 训练集词典版仍需后续单独补跑。
- `BERT-MRC` / `BERT-MRC+DSC`：`_9LOGS/dice_loss_redjujube_train.log`
  显示 `_5TRAIN/tasks/mrc_ner/train.py` 在 `compute_loss` 抛出 `ValueError`，
  未得到 test P/R/F1。
- `RA_NER / AdaSeq BERT-CRF`：`_9LOGS/adaseq_redjujube_train.log`
  显示 AdaSeq 数据集生成阶段因 BMES 标签序列断言失败而中止，未得到 test P/R/F1。
