...
from eznlp.training import LRLambda, Trainer, check_param_groups, collect_params
...
def add_base_arguments(parser: argparse.ArgumentParser):
    ...
    group_debug.add_argument(
        "--pdb", default=False, action="store_true", help="whether to use pdb for debug"
    )
    group_debug.add_argument(
        "--profile", default=False, action="store_true", help="whether to profile"
    )
    group_debug.add_argument(
        "--no_log_terminal",
        dest="log_terminal",
    ...
    group_train.add_argument("--seed", type=int, default=515, help="random seed")
    group_train.add_argument(
        "--use_amp", default=False, action="store_true", help="whether to use amp"
    )
    group_train.add_argument(
        "--train_with_dev",
        default=False,
    ...
    group_train.add_argument("--batch_size", type=int, default=64, help="batch size")
    group_train.add_argument(
        "--grad_clip",
        type=float,
        default=5.0,
    ...
    group_train.add_argument(
        ...
        default="AdamW",
        help="optimizer",
        choices=["AdamW", "SGD", "Adadelta", "Adamax"],
    )
    group_train.add_argument("--lr", type=float, default=0.001, help="learning rate")
    group_train.add_argument(
        "--finetune_lr", type=float, default=2e-5, help="learning rate for finetuning"
    )
    group_train.add_argument(
        "--scheduler",
        type=str,
    ...
    group_model.add_argument(
        ...
        type=str,
        default="None",
        choices=["None", "LSTM", "GRU", "Conv"],
        help="character-level encoder architecture (None for w/o character-level encoder)",
    )
    group_model.add_argument(
        "--use_bigram", default=False, action="store_true", help="whether to use bigram"
    )
    group_model.add_argument(
        "--use_softword",
        default=False,
    ...
    group_model.add_argument(
        ...
        type=str,
        default="LSTM",
        choices=["FFN", "LSTM", "GRU", "Conv", "Gehring", "Transformer"],
        help="token-level encoder architecture",
    )
    group_model.add_argument("--hid_dim", type=int, default=200, help="hidden dim")
    group_model.add_argument(
        "--num_layers", type=int, default=1, help="number of encoder layers"
    )
    group_model.add_argument(
        "--drop_rate", type=float, default=0.5, help="dropout rate"
    )
    group_model.add_argument(
        "--use_locked_drop",
        default=False,
    ...
    group_model.add_argument(
        "--use_elmo", default=False, action="store_true", help="whether to use ELMo"
    )
    group_model.add_argument(
        "--use_flair", default=False, action="store_true", help="whether to use Flair"
    )
    group_model.add_argument(
        "--bert_arch",
        type=str,
    ...
    group_model.add_argument(
        "--bert_drop_rate", type=float, default=0.2, help="dropout rate for BERT"
    )
    group_model.add_argument(
        "--bert_freeze",
        default=False,
...
dataset2language.update(
    {"HwaMei_Privacy": "Chinese", "HwaMei_Privacy_Shaoyang": "Chinese"}
)
...
def load_data(args: argparse.Namespace):
    if args.dataset.startswith("conll2003"):
        ...
        if args.corrupt_rate > 0 and args.doc_level:
            ...
            train_data = json_io.read(
                f"data/conll2003/eng.train.corrupted({args.corrupt_rate:.1f}, 1).json"
            )
            ...
            scores, ave_scores = precision_recall_f1_report(
                set_chunks_gold, set_chunks_corr
            )
            logger.warning(
                f"Loading data with corruption rate of {args.corrupt_rate:.1f} \n"
        ...
        io = ConllIO(
            ...
            tag_col_id=10,
            scheme="OntoNotes",
            sentence_sep_starts=["#end", "pt/"],
            document_sep_starts=["#begin"],
            encoding="utf-8",
            case_mode="None",
            number_mode="Zeros",
        )
        ...
        if args.corrupt_rate > 0:
            set_chunks_gold = [ex["chunks"] for ex in train_data]
            train_data = io.read(
                f"data/ace-lu2015emnlp/ACE2005/train.corrupted({args.corrupt_rate:.1f}, 1).json"
            )
        ...
        io = JsonIO(
            ...
            chunk_start_key="start",
            chunk_end_key="end",
            retain_keys=["doc_key", "bibliomisc"],
            case_mode="None",
            number_mode="Zeros",
        )
        ...
        for idx, chunks_false in set_chunks_false.items():
            train_data[idx]["chunks"] = [
                ck for ck in train_data[idx]["chunks"] if not (ck in chunks_false)
            ]
            train_data[idx]["relations"] = [
                rel
                for rel in train_data[idx]["relations"]
                if not (rel[1] in chunks_false or rel[2] in chunks_false)
            ]
        ...
        train_data = io.read("data/yidu_s4k/subtask1_training_part1.txt") + io.read(
            "data/yidu_s4k/subtask1_training_part2.txt"
        )
        ...
        io = JsonIO(
            ...
            chunk_key="entities",
            chunk_type_key="type",
            chunk_start_key="start_idx",
            chunk_end_key="end_idx",
            encoding="utf-8",
            token_sep="",
            pad_token="",
        )
        ...
        io = JsonIO(
            ...
            attribute_chunk_key="entity",
            relation_key="relations",
            relation_type_key="type",
            relation_head_key="head",
            relation_tail_key="tail",
            is_whole_piece=False,
            retain_keys=["visit_id", "split"],
            encoding="utf-8",
            token_sep="",
            pad_token="",
        )
        ...
        if size > 500:
            ext_data = io.read("data/HwaMei/v20230329/reserve-deid.json") + io.read(
                "data/HwaMei/v20230329/iaa-deid.json"
            )
            ext_ids = set()
        ...
        train_data, dev_data = sklearn.model_selection.train_test_split(
            train_data, test_size=0.2, random_state=args.seed
        )
        ...
        train_data, dev_data = sklearn.model_selection.train_test_split(
            train_data, test_size=0.1, random_state=args.seed
        )
        ...
        io = TextClsIO(
            is_tokenized=False,
            tokenize_callback=jieba.tokenize,
            text_key="text",
            mapping={" ": ""},
            encoding="utf-8",
            verbose=args.log_terminal,
        ...
        dev_data = io.read(
            "data/iwslt14.tokenized.de-en/valid.en",
            "data/iwslt14.tokenized.de-en/valid.de",
        )
        test_data = io.read(
            "data/iwslt14.tokenized.de-en/test.en",
            "data/iwslt14.tokenized.de-en/test.de",
        )

    elif args.dataset == "flickr8k":
        io = KarpathyIO(
            img_folder="data/flickr8k/Flicker8k_Dataset", check_img_path=True
        )
        train_data, dev_data, test_data = io.read(
            "data/flickr8k/flickr8k-karpathy2015cvpr.json"
        )

    elif args.dataset == "flickr30k":
        io = KarpathyIO(
            img_folder="data/flickr30k/flickr30k-images", check_img_path=True
        )
        train_data, dev_data, test_data = io.read(
            "data/flickr30k/flickr30k-karpathy2015cvpr.json"
        )

    elif args.dataset == "mscoco":
        io = KarpathyIO(img_folder="data/mscoco/data2014", check_img_path=True)
        train_data, dev_data, test_data = io.read(
            "data/mscoco/mscoco-karpathy2015cvpr.json"
        )
...
def load_pretrained(pretrained_str, args: argparse.Namespace):
    if pretrained_str.lower() == "elmo":
        ...
        return allennlp.modules.Elmo(
            options_file="assets/allennlp/elmo_2x4096_512_2048cnn_2xhighway_options.json",
        ...
        return (
            flair.models.LanguageModel.load_language_model(
                "assets/flair/news-forward-0.4.1.pt"
            ),
            flair.models.LanguageModel.load_language_model(
                "assets/flair/news-backward-0.4.1.pt"
            ),
        )
        ...
        if pretrained_str.lower().startswith("bert"):
            if "wwm" in pretrained_str.lower():
                PATH = (
                    "assets/transformers/bert-large-cased-whole-word-masking"
                    if args.bert_cased
                    else "assets/transformers/bert-large-uncased-whole-word-masking"
                )
            elif "base" in pretrained_str.lower():
                PATH = (
                    "assets/transformers/bert-base-cased"
                    if args.bert_cased
                    else "assets/transformers/bert-base-uncased"
                )
            elif "large" in pretrained_str.lower():
                PATH = (
                    "assets/transformers/bert-large-cased"
                    if args.bert_cased
                    else "assets/transformers/bert-large-uncased"
                )
            ...
            return (
                transformers.DebertaModel.from_pretrained(
                    PATH,
                    hidden_dropout_prob=args.bert_drop_rate,
                    attention_probs_dropout_prob=args.bert_drop_rate,
                ),
                transformers.DebertaTokenizer.from_pretrained(
                    PATH, add_prefix_space=True
                ),
            )
            ...
            return (
                ...
                transformers.BertTokenizer.from_pretrained(
                    PATH, model_max_length=512, do_lower_case=False
                ),
            )

        elif pretrained_str.lower().startswith("scibert"):
            PATH = (
                "assets/transformers/allenai/scibert_scivocab_cased"
                if args.bert_cased
                else "assets/transformers/allenai/scibert_scivocab_uncased"
            )
            return (
                transformers.BertModel.from_pretrained(
                    PATH,
                ...
                transformers.BertTokenizer.from_pretrained(
                    PATH, model_max_length=512, do_lower_case=(not args.bert_cased)
                ),
            )
            ...
            return (
                ...
                transformers.BertTokenizer.from_pretrained(
                    PATH, model_max_length=512, do_lower_case=True
                ),
            )
...
def load_vectors(
    language: str, emb_dim: int, unigram: bool = False, bigram: bool = True
):
    if language.lower() == "english":
        ...
        if unigram and emb_dim == 50:
            return Vectors.load(
                "assets/vectors/gigaword_chn.all.a2b.uni.ite50.vec", encoding="utf-8"
            )
        elif bigram and emb_dim == 50:
            return Vectors.load(
                "assets/vectors/gigaword_chn.all.a2b.bi.ite50.vec", encoding="utf-8"
            )
        else:
            if emb_dim == 50:
                ...
                return Vectors.load(
                    "assets/vectors/tencent/Tencent_AILab_ChineseEmbedding.txt",
                    encoding="utf-8",
                    skiprows=0,
                )
    return None
...
def profile(trainer, dataloader):
    ...
    sort_by = (
        "cuda_time_total"
        if trainer.device.type.startswith("cuda")
        else "cpu_time_total"
    )
