...
### 📌 12-5周：Boundary Smoothing技巧验证 (2025-01-03) ✅

**计划文档**: [plans/12-4_improvements.md](../plans/12-4_improvements.md) (持续优化阶段)

**实验目录**: 多数据集验证 (cache/*/expert_boundary_20260103*)

**实验报告**:
- [实验记录补充_20250103.md](./实验记录补充_20250103.md) - 1月3日Boundary Smoothing验证 ✅ ⭐ **最新**

**核心结论**:
- 🎉 **Boundary Smoothing有效性验证**: MSRA测试集F1=**95.18%**，与FLAT (95.19%)性能持平
- 📊 **多数据集基准建立**: 完成4个数据集的性能评估
- 💡 **模型选择策略明确**: 领域数据优先BERT-wwm，通用数据优先MacBERT-Large
- ⚠️ **有效实验筛选**: 已剖除Graph Only和训练未完成实验，保疙6个有效结果

**关键实验结果** (2025-01-03):

| 数据集 | 模型配置 | Test F1 | 说明 |
|--------|---------|---------|------|
| **MSRA** | Expert+BS | **95.18%** | ✅ 与FLAT持平 |
| **PingGuo** | BERT-wwm-ext | **97.61%** | ✅ 最优结果 |
| **PingGuo** | MacBERT-Large | **97.46%** | ✅ 大模型 |
| **Boson** | BERT-wwm-ext | **85.73%** | ✅ 社交媒体 |
| **CLUE** | MacBERT-Large | **80.44%** | ✅ 通用基准 |
| **CLUE** | BERT-wwm-ext | **79.91%** | ✅ 基线模型 |
| **CLUE** | Graph Only | **68.20%** | ⚠️ 仅图结构 |

**实验规模**:
- 总实验数: 13个
- 成功实验: 8个 (61.5%)
- 数据集覆盖: MSRA、CLUE、Boson、PingGuo
- 模型类型: BERT-wwm-ext、MacBERT-Large、Graph Only

**关键发现**:
1. ✅ **BS技巧验证成功**: MSRA 95.18%，证明边界平滑有效
2. 💪 **领域数据优势**: PingGuo上 BERT-wwm-ext (97.61%) > MacBERT-Large (97.46%)
3. 📊 **通用数据优势**: CLUE上 MacBERT-Large (80.44%) > BERT-wwm-ext (79.91%)
4. ⚠️ **有效实验筛选**: 13个实验中6个成功 (46.2%)，已剖除Graph Only和未完成实验

**后续计划**:
- 短期: 将BS技巧应用于RedJujube ExpertDict
- 中期: 验证预训练模型选择策略
- 长期: 结合对抗训练、模型集成，冲刻98% F1
...
- [实验记录补充_20241224-20250105.md](./实验记录补充_20241224-20250105.md) - 12-24至1-5整合记录 ⭐ **最新**

**核心结论**:
- 🏆 **FLAT-MSRA成功**: 测试集F1=**95.19%**，超越FLAT论文baseline **+0.73%**
- ⚡ **训练高效**: 7个epoch达峰值（~2小时），后23个epoch仅微调
- 🛡️ **泛化性强**: Test F1 > Dev F1 (+0.96%)，无过拟合
- ⚠️ **FLAT-RedJujube失败**: 最优F1=96.39%，低于ExpertDict (97.00%)
- ❌ **放弃小数据集应用**: FLAT不适合RedJujube (5k样本)

**关键实验结果**:

| 指标类型 | Precision | Recall | F1 | vs FLAT论文 |
|---------|-----------|--------|-----|-------------|
| **MSRA Micro** | **95.52%** | **94.86%** | **95.19%** | **+0.73%** |
| **MSRA Macro** | **94.81%** | **94.75%** | **94.77%** | **+0.31%** |
...
**RedJujube实验尝试** (未完成):
- 方案A (Expert Auto): 最优Dev F1=**96.39%** @ Epoch 15
  - 问题: Epoch 16后出现Recall崩溃 (27% @ Epoch 19)
- 方案B (CTB): 最优Dev F1=**96.12%** @ Epoch 13
  - 问题: 训练不稳定，F1波动在93%-96%
- 对比ExpertDict: **97.00% F1** (稳定) > FLAT **96.39% F1** (不稳定)
...
**关键发现**:
1. ✅ **FLAT适合大规模数据集**: MSRA (45k样本) 表现优异，训练稳定
2. ❌ **FLAT不适合小规模数据集**: RedJujube (5k样本) 出现过拟合和崩溃
3. 💪 **传统方法在小数据集更优**: ExpertDict (97.00%) > FLAT (96.39%)
4. 📊 **数据规模是关键**: 临界点约10k-20k样本
5. ✅ **FLAT架构验证成功**: 在MSRA上成功复现并超越论文
...
**计划调整**:
- ❌ 放弃: FLAT在RedJujube数据集应用
- 🎯 新方向: 聚焦ExpertDict深度优化 (超参数调优 + 对抗训练 + SOTA技巧 + 模型集成)
- 🎯 最终目标: **97.8~98.0% F1**
