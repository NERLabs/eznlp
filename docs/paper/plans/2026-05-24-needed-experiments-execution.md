...
与 `AdaSeq BERT-CRF` 当前路径 RJND/RedJujube seed=42 实验。
...
- `BERT-MRC+DSC`：旧 `ValueError` 与随机矩阵设备问题已修复，`dice_ohem=0`
  + `train_batch_size=4` 的 20-step GPU smoke 已通过。完整 10 epoch 长实验正在
  tmux `rjnd-mrc-dsc-20260524` 中运行，输出目录为
...
- `NFLAT` 暂不直接启动训练：`projects/NFLAT4CNER-main/main.py` 仅支持
  ...
  `redjujube`、`seed`、`n_epochs`、`refresh_data` 与当前向量路径；`flat37`
  已补 `prettytable` 并可 import FastNLP。完整训练仍需等待 GPU 空闲。
