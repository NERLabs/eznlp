...
- `BERT-MRC+DSC` is no longer blocked by the old `span_loss_candidates`
  ...
  Full current RedJujube Dice/DSC training is running in tmux session
  ...
  Do not register a table value until `eval_result_log.txt` contains final test P/R/F1.
- `BERT-MRC+DSC` with `dice_ohem=0.3` still fails on the current server:
  ...
  `train_batch_size=10`. The running configuration disables OHEM and reduces
...
- `NFLAT` source is available under `references/external_projects/NFLAT4CNER-main`.
  ...
  `refresh_data`, and current vector paths; `flat37` now imports FastNLP after
  installing `prettytable`. A full run is still not started while MRC/DSC is
  using GPU.
