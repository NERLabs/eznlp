...
# -*- coding: utf-8 -*-
"""
MSRA 数据集 NER 训练脚本 - 完整方法对比实验

对比所有方法（与RedJujube对标）：
1. Baseline: MacBERT + BiLSTM + CRF
2. SoftLexicon: MacBERT + BiLSTM + CRF + 软词典特征
3. ExpertDict: MacBERT + BiLSTM + CRF + 专家词典特征
4. Fusion: Soft+Expert融合（Concat/Weighted/Gated/Attention）
"""
...
def load_msra_data(data_dir, max_seq_len=450):
    """加载 MSRA 数据集，截断过长序列"""
    io = ConllIO(
        text_col_id=0,
        tag_col_id=1,
    ...
    train_data = io.read(f"{data_dir}/train.char.bmes")
    dev_data = io.read(f"{data_dir}/dev.char.bmes")
    test_data = io.read(f"{data_dir}/test.char.bmes")
...
def main():
    ...
    # 数据参数
    parser.add_argument('--data_dir', type=str, default='datasets/raw/MSRA',
                        help='数据目录')
    parser.add_argument('--expert_dict_path', type=str, default='datasets/raw/MSRA/expert_lexicon_auto.txt',
                        help='手动专家词典路径')
    parser.add_argument('--expert_dict_auto_path', type=str, default='datasets/raw/MSRA/expert_lexicon_auto.txt',
                        help='自动专家词典路径')
    parser.add_argument('--save_dir', type=str, default='cache/msra_ner_comparison',
                        help='保存目录')
    ...
    parser.add_argument('--softlex_train_path', type=str, default='datasets/raw/MSRA/softlexicon_train.txt',
    ...
    train_data, dev_data, test_data = load_msra_data(args.data_dir, max_seq_len=450)
