...
import numpy
import torch
from research.tools.utils import (
    add_base_arguments,
    build_trainer,
    dataset2language,
    header_format,
...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_data.add_argument(
        "--dataset", type=str, default="conll2003", help="dataset name"
    )
    group_data.add_argument(
        "--doc_level",
        default=False,
    ...
    group_data.add_argument(
        "--corrupt_rate", type=float, default=0.0, help="boundary corrupt rate"
    )
    group_data.add_argument(
        "--remove_nested",
        default=False,
    ...
    group_data.add_argument(
        "--jackknife", type=int, default=-1, help="the 5-fold index of jackknifing"
    )
    group_data.add_argument(
        "--no_save_preds",
        dest="save_preds",
    ...
    group_decoder.add_argument(
        "--size_emb_dim", type=int, default=25, help="span size embedding dim"
    )
    group_decoder.add_argument(
        "--inex_mkmmd_lambda",
        type=float,
    ...
    group_decoder.add_argument(
        "--inex_chunk_priority_training",
        type=str,
        default="confidence",
        help="chunk priority in the training phase",
    )
    group_decoder.add_argument(
        "--inex_chunk_priority_testing",
        type=str,
        default="confidence",
    ...
    group_decoder.add_argument(
        "--sb_epsilon", type=float, default=0.0, help="boundary smoothing loss epsilon"
    )
    group_decoder.add_argument(
        "--sb_size", type=int, default=1, help="boundary smoothing window size"
    )
    group_decoder.add_argument(
        "--sb_adj_factor",
        type=float,
    ...
    group_decoder.add_argument(
        ...
        type=int,
        default=2,
        help="minimum span size",
        choices=[2, 1],
    )
    group_decoder.add_argument(
        "--sse_max_span_size_cov_rate",
        type=float,
        default=0.995,
...
def collect_IE_assembly_config(args: argparse.Namespace):
    ...
    if args.emb_dim > 0:
        ...
        ohots_config["text"] = OneHotConfig(
            field="text", vectors=vectors, emb_dim=args.emb_dim, freeze=args.emb_freeze
        )

    if args.language.lower() == "chinese" and args.use_bigram:
        vectors = load_vectors(args.language, 50, bigram=True)
        ohots_config["bigram"] = OneHotConfig(
            field="bigram", vectors=vectors, emb_dim=50, freeze=args.emb_freeze
        )
    ...
    if args.bert_arch.lower() != "none":
        ...
        bert_like_config = BertLikeConfig(
            ...
            bert_like=bert_like,
            arch=args.bert_arch,
            from_subtokenized=args.pre_subtokenize,
            freeze=args.bert_freeze,
        )

        if (
            getattr(args, "ck_decoder", "").startswith("specific_span")
            or getattr(args, "rel_decoder", "").startswith("specific_span")
            or getattr(args, "attr_decoder", "").startswith("specific_span")
        ):
            bert_like_config.output_hidden_states = True
            span_bert_like_config = SpanBertLikeConfig(
                bert_like=bert_like,
                arch=args.bert_arch,
                freeze=args.bert_freeze,
        ...
        if (
            getattr(args, "ck_decoder", "").startswith("masked_span")
            or getattr(args, "rel_decoder", "").startswith("masked_span")
            or getattr(args, "attr_decoder", "").startswith("masked_span")
        ):
            bert_like_config.output_hidden_states = True
            masked_span_bert_like_config = MaskedSpanBertLikeConfig(
                bert_like=bert_like,
                arch=args.bert_arch,
                freeze=args.bert_freeze,
                num_layers=None if args.sse_num_layers < 0 else args.sse_num_layers,
...
def build_ER_config(args: argparse.Namespace):
    ...
    if args.ck_decoder == "sequence_tagging":
        ...
        decoder_config = SpecificSpanClsDecoderConfig(
            ...
            max_span_size=(
                args.sse_max_span_size if args.sse_max_span_size > 0 else None
            ),
            size_emb_dim=args.size_emb_dim,
            inex_mkmmd_lambda=args.inex_mkmmd_lambda,
            in_drop_rates=drop_rates,
    ...
    if args.ck_decoder == "specific_span":
        return SpecificSpanExtractorConfig(
            **collect_IE_assembly_config(args),
            share_interm2=args.sse_share_interm2,
...
def process_IE_data(train_data, dev_data, test_data, args, config):
    if config.bert_like is not None:
        ...
        if args.doc_level and (
            ...
            train_data = preprocessor.merge_sentences_for_data(
                train_data, doc_key=doc_key
            )
            dev_data = preprocessor.merge_sentences_for_data(dev_data, doc_key=doc_key)
            test_data = preprocessor.merge_sentences_for_data(
    ...
    if args.use_softword or args.use_softlexicon:
        if (
            config.nested_ohots is not None
            and "softlexicon" in config.nested_ohots.keys()
        ):
    ...
    if getattr(args, "remove_nested", False):
        ...
        for data in [train_data, dev_data, test_data]:
            for entry in data:
                ...
                entry["chunks"] = list(
                    set(entry["chunks"]) - set(entry["chunks_nested"])
                )
...
def conll2003nff_post_process(chunks):
    processed = []
    for ck1 in chunks:
        if ck1[0] == "ORG" and any(
            _is_ordered_nested(ck1, ck2) and (ck1 != ck2) for ck2 in chunks
        ):
            processed.append(("LOC", *ck1[1:]))
        elif ck1[0] == "PER" and any(
            (ck2[0] == "PER") and _is_ordered_nested(ck1, ck2) and (ck1 != ck2)
            for ck2 in chunks
        ):
            continue
        else:
...
if __name__ == "__main__":
    ...
    if args.jackknife >= 0:
        ...
        doc_key = "doc_key"
        doc_keys = numpy.unique(
            [entry[doc_key] for entry in train_data]
        )  # Sorted order
    ...
    evaluate_entity_recognition(
        trainer, dev_set, batch_size=args.batch_size, eval_inex=args.eval_inex
    )
    ...
    evaluate_entity_recognition(
        ...
        test_set,
        batch_size=args.batch_size,
        eval_inex=args.eval_inex,
        pp_callback=dataset2pp_callback.get(args.dataset, None),
    )
